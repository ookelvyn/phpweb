...
$hashed_password = password_hash($password, PASSWORD_DEFAULT); // this function works only in PHP 5.5 or latest version
 ...
 $check_email = $conn->query("SELECT email FROM adminlogin WHERE email='$email'");
 ...
 if ($count==0) {
  ...
  $query = "INSERT INTO adminlogin (name, username, email, password) VALUES('$name', '$username', '$email', '$hashed_password')";
